...
class BFT(_BaseDataset):
    """Dataset class for BFT tracking"""
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            'GT_FOLDER': os.path.join(code_path, 'data/gt/bft/'),  # Location of GT data
            'TRACKERS_FOLDER': os.path.join(code_path, 'data/trackers/bft/'),  # Trackers location
            'OUTPUT_FOLDER': os.path.join(code_path, 'output'),  # Where to save eval results (if None, same as TRACKERS_FOLDER)
            'DATA_FOLDER': './data/bft/test',
            'TRACKERS_TO_EVAL': None,  # Filenames of trackers to eval (if None, all in folder)
            'CLASSES_TO_EVAL': ['bird'],  # Valid: ['pedestrian']
            'BENCHMARK': 'BFT',  # Valid: 'MOT17', 'MOT16', 'MOT20', 'MOT15'
    ...
    def __init__(self, config=None):
        ...
        if not all(self.class_list):
            raise TrackEvalException('Attempted to evaluate an invalid class. Only bird class is valid.')
    ...
    def _get_seq_info(self):
        ...
        for seq in seq_list:
            # seq_gt = os.path.join(self.config['GT_FOLDER'], seq)
