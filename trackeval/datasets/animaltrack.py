...
class AnimalTrack(_BaseDataset):
    """Dataset class for AnimalTrack tracking"""
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            'GT_FOLDER': os.path.join(code_path, 'data/gt/animaltrack/'),  # Location of GT data
            'TRACKERS_FOLDER': os.path.join(code_path, 'data/trackers/animaltrack/'),  # Trackers location
            'OUTPUT_FOLDER': os.path.join(code_path, 'output'),  # Where to save eval results (if None, same as TRACKERS_FOLDER)
            'DATA_FOLDER': './data/animaltrack/frames_all',
    ...
    def _get_seq_info(self):
        ...
        known_classes = ['horse', 'zebra']
        unkown_classes = ['chicken', 'deer', 'dolphin', 'duck', 'goose', 'penguin', 'pig', 'rabbit']
        if self.subset == 'known':
            seq_list = [seq for seq in seq_list if seq.split('_')[0] in known_classes]
        elif self.subset == 'unknown':
            seq_list = [seq for seq in seq_list if seq.split('_')[0] in unkown_classes]
        for seq in seq_list:
            seq_dir = os.path.join(self.config['DATA_FOLDER'], seq[:-4])
